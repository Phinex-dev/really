...
`,t=`<script data-freeform-compat="summary">\n${e}\n<\/script>`;function n(e){let n=String(e||``);if(!n.trim())return``;if(/data-freeform-compat=["']summary["']/i.test(n))return n;if(/<head\b[^>]*>/i.test(n))return n.replace(/<head\b[^>]*>/i,e=>`${e}\n${t}`);if(/<body\b[^>]*>/i.test(n))return n.replace(/<body\b[^>]*>/i,e=>`${e}\n${t}`);let r=n.match(/^\s*<!doctype[^>]*>/i);if(r){let e=r[0].length;return`${n.slice(0,e)}\n${t}${n.slice(e)}`}return`${t}\n${n}`}var r=`<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
